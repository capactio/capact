...
func NewUpgrade() *cobra.Command {
	...
	flags.StringVar(&opts.Parameters.Override.HelmRepoURL, "helm-repo-url", upgrade.CapactioHelmRepoOfficial, fmt.Sprintf("Capact Helm chart repository URL. Use %s tag to select repository which holds latest Helm chart versions.", upgrade.CapactioHelmRepoLatestTag))
