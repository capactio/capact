...
import { OnCreateRequest, OnDeleteRequest } from './proto/ts-plugin/storage_backend';
...
async function main() {
  ...
  const createRes = await svc.onCreate(onCreate).catch((err: ServiceError) => console.error(err));
  if (createRes) {
  ...
  const getRes = await svc.getValue(onGet).catch((err: ServiceError) => console.error(err));
  if (getRes) {
    console.log(`Fetch TypeInstance: ${getRes.value}`);
  }

  const onDel: OnDeleteRequest = ti;
  await svc.onDelete(onDel).then(() => console.info('Deleted TypeInstance')).catch((err: ServiceError) => console.error(err));
}
...
export default class DelegatedStorageClient {
  ...
  async getValue(req: GetValueRequest): Promise<GetValueResponse__Output | undefined> {
  ...
  async onCreate(req: OnCreateRequest): Promise<OnCreateResponse__Output | undefined> {
    return new Promise((resolve, reject) =>
  ...
  async onDelete(req: OnDeleteRequest): Promise<OnDeleteResponse__Output | undefined> {
    return new Promise((resolve, reject) =>
