...
> You can notice, that `confluence-config` (which is the `additional` output TypeInstance from `helm.run`) is defined in the `outputTypeInstanceRelations`, although is was created in `helm.run`. The `additional` from `helm.run` is specially, because `helm.run` does not know the Type of TypeInstances, so it's not defined in `helm.run` Implementation, but must be defined in the caller Implementation. In the future, we will improve the syntax, so it will be more clear, which TypeInstances need a seperate entry in `outputTypeInstanceRelations` and which don't.
