...
1. Clone the `release-0.3` branch from the `go-voltron` repository.
	
	```bash
	git clone --depth 1 --branch release-0.3 https://github.com/Project-Voltron/go-voltron.git
	cd ./go-voltron
	```
...
For example, to use manifests Capact `release-0.3` branch, run:
...
export BRANCH_NAME=release-0.3
